...
MSM v0.10.0은 6개 스킬이 5-Layer에서 협업합니다. `msm-orchestration`이 진입점이며, 서브스킬은 `workflow/*.yaml`을 통해 on-demand로 실행됩니다.
...
**무엇을 하나:** evidence 후보에서 entity·relation을 생성하고 MECE를 검증합니다. `evidence/` 후보를 받아 `ontology/explain/concept/` · `ontology/explain/instance/`에 승격하고 `canonical_root_hub.yaml`을 갱신합니다.
...
> **v0.1x 예정:** `msm-graph-reasoning` (multi-hop·BFS·GraphRAG·RDF/OWL), `msm-semantic-search` (zvec·RRF)
