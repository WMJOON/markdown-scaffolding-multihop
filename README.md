# markdown-scaffolding-multihop (v0.1.5)
...
`md-ralph-etl` — URL이나 로컬 문서를 크롤링해서 KB에 새 증거 노드를 추가할 때 씁니다. 웹 페이지, 논문, 기사를 읽어 `evidence/[topic]/sources/`에 구조화된 노트로 넣습니다. v0.1.5부터 arxiv 등 PDF URL 직접 처리를 지원합니다(`step_pdf.py`).
...
`md-obsidian-cli` — Claude가 Obsidian vault의 노트를 직접 읽고 쓰고 검색해야 할 때 씁니다. 노트 CRUD, 태그 검색, 플러그인 제어를 처리합니다. L0/L1/L2 허브-리프 구조 등 Obsidian 그래프 계층 설계 패턴도 포함합니다.
...
| 개선 영역 | v0.1.4 | v0.1.5 |
|-----------|--------|--------|
| `md-obsidian-cli` SKILL.md | 독립형 커맨드 레퍼런스 (~190줄) | **네비게이션 허브로 재구조화** (~45줄) — 중복 내용 제거, 모듈·참조 파일로 분산 |
| Obsidian 계층 구조 패턴 | 없음 | **`references/graph-hierarchy-patterns.md` 신규** — L0/L1/L2 허브 구조, wikilink 패턴, md-graph-multihop 연동 가이드 |
| `md-scaffolding-design` ↔ 계층 패턴 충돌 | `parent:` 스키마 불일치 | `parent: "[[...]]"` 단일 필드로 통일 → `relation_map: {parent: child_of}` 직접 호환 |
| `md-ralph-etl` PDF 지원 | HTML 크롤링 전용 | **`step_pdf.py` 신규** — arxiv 등 PDF URL 직접 처리 (opendataloader-pdf / pymupdf4llm fallback) |
| `md-data-analysis` | `correlation_analysis.py` f-string 구문 오류 | **수정 완료** — Python 3.11 이하 호환 |
