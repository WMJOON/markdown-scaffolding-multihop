...
## v0.13.4 Record Archive · Explain 정렬

v0.13.4는 test repo에서 확정한 `record-archive/`와 `msm-explain` 구조를 실제 공개 스킬팩에 반영한 패치다.

- `msm-record-archive`가 runtime DB, event, derived record, snapshot의 canonical 스킬이다.
- `msm-explain`이 `record-archive/snapshots/`를 `ontology/explain/` Markdown/Base projection으로 렌더링하는 canonical 스킬이다.
- `msm-instance`와 `msm-obsidian-projection`은 호환 wrapper로만 유지한다.
- 실제 row/event/state는 `ontology/` 밖의 `record-archive/`에 저장한다.
- `ontology/system/**/*.ttl`은 형식 그래프이고, TBox/RBox는 파일명이 아니라 TTL 내부 RDF/OWL 구문으로 구분한다.
- Evidence 출처는 `source_refs`에서 PROV-O projection(`*.prov.ttl`, `*.prov.shapes.ttl`)으로 매핑한다.
- 시간축은 `published_at`, `observed_at`, `captured_at`, `archived_at`, `valid_at/effective_at`, `derived_at`로 분해한다.
...
- 설치 목록에 canonical 스킬과 legacy wrapper를 함께 포함해 README의 스킬 구성과 실제 설치 결과를 정렬했다.
...
```
Layer 1 — Repository    canonical_root_hub.yaml · ontology/ · evidence/ · record-archive/ · agent-context/
...
10개 스킬이 5-Layer에서 협업한다. `msm-orchestration`이 진입점이며, 서브스킬은 `agent-context/workflow`의 ABox TTL을 정본으로 삼고 legacy YAML은 migration layer로 소비한다. 이 중 `msm-instance`, `msm-obsidian-projection`은 legacy alias다.
...
flowchart LR
    ...
    subgraph record["Layer 2.4 — Record Archive"]
        rec["msm-record-archive<br/><small>runtime DB · events · derived · snapshots</small>"]
        exp["msm-explain<br/><small>snapshots → ontology/explain projection</small>"]
    end
...
v0.13.4  Record Archive · Explain canonical skill 승격                   ← 현재
