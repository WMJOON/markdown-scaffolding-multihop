#!/usr/bin/env bash
# msm-ontology skill-level harness entrypoint.
...
rc=0
if [[ "$SKILL" == "msm-ontology" && "$TIER" == "L0" ]]; then
  "$PY" "$L0/validate_tbox.py" --target "$TARGET" || rc=$?
  "$PY" "$L0/validate_abox.py" --target "$TARGET" || rc=$?
