...
> **전제:** 온톨로지 분해 전에 Evidence를 먼저 수집한다.
> `evidence/[topic]/sources|notes|claims/` 에 근거를 쌓은 뒤 ETL을 시작한다.

---

## 4단계 프로세스 (Evidence 선행 포함)

### Step 0: Evidence 수집 (ETL 전제)

Ontology에 올릴 내용은 반드시 Evidence에서 온다.

```text
evidence/[topic]/
  sources/   ← 원문, 논문, 문서
  notes/     ← 요약 메모, 발췌
  claims/    ← claim-to-source 매핑, 검증 포인트
```
...
**관계 정의 위치:** `schema/relation/[relation-type].yaml` — `ontology/` 밖에 둔다.
Obsidian `path:ontology/` 필터에 섞이지 않고, Neo4j relationship type 스키마로 직접 사용된다.
yaml 스키마 상세 → `module.kb-structure.md` 참조.

**관계 유형:**

| 유형 | 예시 | graph-ontology 표현 | schema 파일 |
|------|------|---------------------|------------|
| 계층 (is-a) | `Engineer` is-a `Person` | `subClassOf` | `schema/relation/is-a.yaml` |
| 부분-전체 (part-of) | `Task` part-of `Project` | `object_properties.part_of` | `schema/relation/part-of.yaml` |
| 연관 (related-to) | `Meeting` mentions `Person` | `object_properties.mentions` | `schema/relation/mentions.yaml` |
| 참조 (depends-on) | `Document` depends-on `Concept` | `object_properties.depends_on` | `schema/relation/depends-on.yaml` |
| 순서 (follows) | `Task` follows `Task` | `object_properties.follows` | `schema/relation/follows.yaml` |
...
상태 정의 및 승격 조건 상세 → `module.kb-structure.md` 참조.

ontology instance 파일은 반드시 `status` 필드를 가진다: `draft` → `experimental` → `validated` (또는 `deprecated`).
