...
```
1. evidence 수집
   → evidence/[domain]/sources/ 에 새 논문 노트 추가
...
```
1. raw/ 스캔
   → raw/[domain]/ 아래 처리되지 않은 문서 목록 수집
   → frontmatter에 status: raw 또는 processed 없음 확인

2. 분류 (Diagnose)
   → 각 raw 문서에 대해:
     a. concept 노드로 승격 가능한가? → ontology/instance/[domain]/[instance].md (개념 정의는 ontology/concept/[domain]/[Concept].md)
     b. evidence 노드인가? → evidence/[domain]/sources/
