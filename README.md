# MSM — Human-Agent KnowledgeBase Management System (v0.13.2)
...
8개 스킬이 5-Layer에서 협업한다. `msm-orchestration`이 진입점이며, 서브스킬은 workflow TTL을 정본으로 삼고 legacy YAML은 migration layer로 소비한다.
...
- `workflow/*.ttl` — workflow 정본. orchestration/harness가 우선 소비
- `*-workflow.yaml` — legacy migration layer. TTL 정본과 동기화 검증 대상
...
| [워크플로우](docs/guides/workflows.md) | workflow TTL 카테고리, legacy YAML 마이그레이션, 스킬 바인딩 |
...
v0.13.2  Provider-Free install 정렬 · Codex 스킬 설치 누락 보완          ← 현재
